...
export class CollapseAnimationDirective implements OnInit, AfterContentInit, OnDestroy {
  ...
  @Input() public collapseAnimationTiming = '';
  @Input('jhiCollapseAnimation') public set _isCollapsed(value: boolean) {
  ...
  @Output('jhiCollapseAnimation.start') public startEventEmitter = new EventEmitter();
  // tslint:disable-next-line:no-output-rename
  @Output('jhiCollapseAnimation.done') public doneEventEmitter = new EventEmitter();
   ...
   */
  constructor(private elementRef: ElementRef, private ngZone: NgZone, private renderer2: Renderer2) {}
  ...
  private emit(phaseName: string): void {
    const event: AnimationEvent = {
      ...
      triggerName: 'jhiCollapseAnimation',
    };
